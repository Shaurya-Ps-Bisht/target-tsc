...
// Assume these libraries exist and are correctly implemented.
...
function scrubIgnoredFields(actual: any, expected: any): { scrubbedActual: any, scrubbedExpected: any } {
  ...
  function traverse(objActual: any, objExpected: any) {
    for (const key in objExpected) {
      if (objExpected[key] === "@Ignore@") {
        delete objActual[key];
        delete objExpected[key];
      } else if (typeof objExpected[key] === 'object' && objExpected[key] !== null && typeof objActual[key] === 'object' && objActual[key] !== null) {
        traverse(objActual[key], objExpected[key]);
      }
    }
  }
